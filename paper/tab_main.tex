...
two-sided exact McNemar test on the discordant pairs. Notice that no
comparison reaches significance and that six of the seven $\Delta$
intervals contain zero. The exception is TituLLM-1B-Instruct, whose
interval is computed from only eight discordant pairs; the Wald interval
is anti-conservative at that count and the exact test ($p=0.070$) is the
reliable reading.}
\label{tab:main}
\centering\small
